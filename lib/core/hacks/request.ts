...
export const hack = <T extends typeof http.request>(
  ...
  logger.debug(`Request begin. ${method} ${hostname}${port ? `:${port}` : ""} ~ ${path}`);
  ...
  const finishRequest = (): void => {
    ...
    context.captureSN += 1;
    logger.debug(
      `Record request info. Response body length: ${requestLog.responseLength}`
    );
  };
  ...
  request.once("socket", (socket: Socket): void => {
    ...
    if (!isIP(hostname)) {
      ...
      socket.once(
        ...
        (err: Error, address: string, family: string | number, host: string): void => {
          ...
          if (err) {
            logger.error(`Lookup error ${err.stack}`);
          }
        }
      );
    }
    ...
    if (socket.remoteAddress) {
      ...
      logger.debug(`Socket reused. Remote: ${socket.remoteAddress}:${socket.remotePort}`);
    }
  });
  ...
  request.once("finish", () => {
    ...
    if (tooLarge) {
      requestBody = Buffer.from(`body was too large too show, length: ${length}`).toString(
        "base64"
      );
    } else {
    ...
    logger.debug(
      `Request send finish. Body size ${length}. Cost: ${
  ...
  request.once("response", (response: http.IncomingMessage): void => {
    ...
    logger.debug(
      `Request on response. Socket chain: ${socket.localAddress}:${socket.localPort} > ${
        socket.remoteAddress
      }:${socket.remotePort}. Response status code: ${response.statusCode}. Cost: ${
        timestamps.onResponse.getTime() - timestamps.onSocket.getTime()
      } ms`
    );
    ...
    response.once("end", () => {
      ...
      logger.debug(
        `Response on end. Body size：${requestLog.responseLength}. Cost: ${
          timestamps.responseClose.getTime() - timestamps.onSocket.getTime()
        } ms`
      );
