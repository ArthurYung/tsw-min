...
export const dnsHack = (): void => {
  ...
  if (!dnsHacked) {
    ...
    dns.lookup = ((lookup) => (
      ...
      const callback =
        typeof optionsOrCallback === "function" ? optionsOrCallback : callbackOrUndefined;
      ...
      const callbackWrap = (
        ...
        if (!err) {
          logger.debug(`dns lookup [${dnsTime}ms]: ${hostname} > ${address}`);
        } else {
          logger.error(`dns lookup [${dnsTime}ms] error: ${err.stack}`);
        }
